...
func (r Service) doRequest(req *http.Request) (string, error) {
	res, err := r.client.Do(req)
	if err != nil {
		return "", errors.New(fmt.Sprintf("error while sending request. %v", err))
	}
	if res.Body != nil {
...
func (r Service) GetHistoricalCurrenciesByPeriod(startDate, endDate, currencies, source string) (string, error) {
	...
	if err != nil {
		return "", errors.New(fmt.Sprintf("error while creating historical period currencies request. %v", err))
	}
...
func (r Service) GetHistoricalCurrencies(date, currencies, source string) (string, error) {
	...
	if err != nil {
		return "", errors.New(fmt.Sprintf("error while creating historical currencies request. %v", err))
	}
...
func (r Service) GetSupportedCurrencies() (string, error) {
	...
	if err != nil {
		return "", errors.New(fmt.Sprintf("error while creating supported currencies request. %v", err))
	}
	reqSupportedCurrencies.Header.Set("api", r.config.ApiKey)
